...
def maze_solver_randomized(maze: BinaryTreeMaze, point: _Cell, path: list[_Cell]) -> Optional[list]:
    """A maze solver detecting a path and returning backwards
    ...
    if indicator == 1:
        ...
        if point.walls[2] == 0 and east.state == 'naive':
            east.state = 'visited'
            point.update_cell_state()
            return maze_solver(maze, east, path)
