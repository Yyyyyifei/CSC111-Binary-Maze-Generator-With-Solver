...
from maze_solver import maze_solver, maze_solver_randomized
CELL = _Cell(set())
SIZE = (700, 700)
WIDTH = 30
...
def run_example5(size: tuple, width: int) -> None:
    """Visualize a path generated from the maze we have produced
    ...
    maze = BinaryTreeMaze(rows, columns)
    maze.binary_tree_algorithm()
    path = maze_solver_randomized(maze, maze.cells[0], [])
    maze.draw_maze(size, path)
